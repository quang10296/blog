...
const RecentProjects = ({ MAX_PROJECTS }) => {
  const projectsList = projectsData.slice(0, 2)

  return (
    <>
      <div className="divide-y divide-gray-700">
        <div className="my-4">
          <span className="font-poppins title-font text-3xl font-bold">Chứng chỉ của tôiiiiiiii</span>
        </div>
        <div className="py-5">
          <AnimatedDiv
        ...
        <div className="mt-5 flex justify-end text-base font-medium leading-6">
          <Link href="/cert" className="mt-5 hover:text-primary-400" aria-label="all posts">
            Toàn bộ chứng chỉ của tôi nè &rarr;
          </Link>
        </div>
      </div>
    </>
